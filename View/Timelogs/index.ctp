<div class="timelogs index">
	<h2><?php echo __('Timelogs'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('task_id'); ?></th>
			<th><?php echo $this->Paginator->sort('user_id'); ?></th>
			<th><?php echo $this->Paginator->sort('date'); ?></th>
			<th><?php echo $this->Paginator->sort('worktime'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($timelogs as $timelog): ?>
	<tr>
		<td><?php echo h($timelog['Timelog']['id']); ?>&nbsp;</td>
		<td><?php echo h($timelog['Timelog']['task_id']); ?>&nbsp;</td>
		<td><?php echo h($timelog['Timelog']['user_id']); ?>&nbsp;</td>
		<td><?php echo h($timelog['Timelog']['date']); ?>&nbsp;</td>
		<td><?php echo h($timelog['Timelog']['worktime']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $timelog['Timelog']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $timelog['Timelog']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $timelog['Timelog']['id']), null, __('Are you sure you want to delete # %s?', $timelog['Timelog']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Timelog'), array('action' => 'add')); ?></li>
	</ul>
</div>
